...
#define FIREBASE_AUTH "UORT4mK732sLp1THDEvm8Yp7zxCtPcBFVLtvCipB"
...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks
{
    ...
    void onResult(BLEAdvertisedDevice advertisedDevice)
    {
      ...
      time_t now; 
      time(&now);
      Firebase.setInt(firebaseData, "/time2", now);
      
      //https://blog.csdn.net/Naisu_kun/article/details/115627629(網路時間)

      if(beacon_device[ID_No] == 5 && beacon_rssi_close_device[ID_No] > -90){
      Firebase.setInt(firebaseData, "/esp32 no_2/" + bUUID + "/time" , now);
      float M_1 = pow(10,((abs(beacon_rssi_close_device[ID_No]) - A) / (10 * n)));
      Serial.println(M_1);
      Serial.println("資料上傳，近距離");

      Firebase.setInt(firebaseData, "/esp32 no_2/" + bUUID + "/RSSI", beacon_rssi_close_device[ID_No]);
        
      Firebase.setFloat(firebaseData, "/esp32 no_2/" + bUUID + "/distance" , M_1); //利用string變數使得可以同時讓firebase存不同的UUID
      ...
      Firebase.setInt(firebaseData, "/esp32 no_2/" + bUUID + "/RSSI", div_RSSI);
