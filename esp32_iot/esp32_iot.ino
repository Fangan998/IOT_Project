...
int beacon_rssi_close_device[100];  //用來存放接近 RSSI 的陣列(-45以內)，如果有該範圍內，他會變動
int beacon_device[100];             //用來存放該 beacon 被偵測次數 之陣列
int beacon_list_avarage[100];       //用來存放 RSSI 累積的陣列，以進行平均 RSSI
int second = 0;
...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks
{
    void onResult(BLEAdvertisedDevice advertisedDevice)
    {
      ...
      if(bUUID == "df5a60394a2e-f41b-fafc-6cbe-70468752")
      {
        ...
        //如果RSSI比-45大...，更新「接近」，反之，累加「累計」
        if((bRSSI >= -45) && (bMajor < 100)){ 
         ...
         if(beacon_device[bMajor] == 3 && beacon_rssi_close_device[bMajor] > -90){
          
            //Serial.println("資料上傳，近距離");

            reconnect();
            Firebase.setInt(firebaseData, "/esp32 no_0/" + String(bMajor) + "/RSSI", beacon_rssi_close_device[bMajor]);
            ...
            reconnect();
            Firebase.setInt(firebaseData, "/esp32 no_0/" + String(bMajor) + "/Major", bMajor);
            Firebase.setInt(firebaseData, "/esp32 no_0/" + String(bMajor) + "/Minor", bMinor);
            Firebase.setInt(firebaseData, "/esp32 no_0/" + String(bMajor) + "/time", second);
            Firebase.setInt(firebaseData, "/esp32 no_0/" + String(bMajor) + "/epochTime_temp", epochTime_temp);
            delay(400);      
         ...
         }else if(beacon_device[bMajor] == 3 && beacon_rssi_close_device[bMajor] <= -90){
          
            int div_RSSI = beacon_list_avarage[bMajor] / 3;
            //Serial.print("資料上傳，遠距離，div_RSSI為：");
            //Serial.println(div_RSSI);

            reconnect();
            Firebase.setInt(firebaseData, "/esp32 no_0/" + String(bMajor) + "/RSSI", div_RSSI);
...
void loop()
{
  ...
  if(epochTime_count == 0){
    ...
    Firebase.setInt(firebaseData, "/esp32 no_0/time/time" , epochTime);
    epochTime_count++;
    delay(1000);
  }
